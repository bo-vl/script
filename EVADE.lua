...
local MiscTab = Window:MakeTab({Name="Extra",Icon="rbxassetid://4483345998",PremiumOnly=false});
local ESPTab = Window:MakeTab({Name="Tracers",Icon="rbxassetid://4483345998",PremiumOnly=false});
local TeleportTab = Window:MakeTab({Name="Teleport",Icon="rbxassetid://4483345998",PremiumOnly=false});
local FunTab = Window:MakeTab({Name="Fun",Icon="rbxassetid://4483345998",PremiumOnly=false});
local TestTab = Window:MakeTab({Name="Test",Icon="rbxassetid://4483345998",PremiumOnly=true});
local MainTab3 = MainTab:AddSection({Name="Sliders"});
...
MainTab:AddSlider({Name="Hip height",Min=-1.45,Max=100,Default=-1.45,Color=Color3.fromRGB(255, 255, 255),Increment=1,Callback=function(HipValue)
	game.Players.LocalPlayer.Character.Humanoid.HipHeight = HipValue;
end});
local FunTab2 = FunTab:AddSection({Name="Toggles"});
local MiscTab3 = MainTab:AddSection({Name="Toggles"});
FunTab:AddToggle({Name="AutoSlide (press c for onces after turning on)",Default=false,Callback=function(Value)
	AutoSlide = Value;
	Autoslide();
end});
FunTab:AddToggle({Name="fake money giver",Default=false,Callback=function(Value)
	money = Value;
	freemoney();
end});
MainTab:AddToggle({Name="auto respawn (you respawn when you get downed)",Default=false,Callback=function(Value)
	revivedie = Value;
	god();
end});
FunTab:AddToggle({Name="Auto Wistle",Default=false,Callback=function(Value)
	autowistle = Value;
	autowistlefunction();
end});
local FunTab3 = FunTab:AddSection({Name="Buttons"});
local MiscTab2 = MiscTab:AddSection({Name="Buttons"});
local Maintab2 = MainTab:AddSection({Name="Buttons"});
MainTab:AddButton({Name="Alpha Skin (Permanant)",Callback=function()
	game:GetService("ReplicatedStorage").Events.UI.Purchase:InvokeServer("Skins", "AlphaTester");
end});
MainTab:AddButton({Name="Boombox Skin (Permanant)",Callback=function()
	game:GetService("ReplicatedStorage").Events.UI.Purchase:InvokeServer("Skins", "Boombox");
end});
MiscTab:AddButton({Name="Inf Jump",Callback=function()
...
MiscTab:AddButton({Name="Q to Teleport",Callback=function()
...
MiscTab:AddButton({Name="Full Bright",Callback=function()
	dofullbright();
end});
MiscTab:AddButton({Name="Return Too Main Menu",Callback=function()
	game:GetService("ReplicatedStorage").Events.ReturnToMenu:FireServer();
end});
MiscTab:AddButton({Name="Low Quality",Callback=function()
	local ohString1 = "LowQuality";
	local ohBoolean2 = true;
	game:GetService("ReplicatedStorage").Events.UpdateSetting:FireServer(ohString1, ohBoolean2);
end});
FunTab:AddButton({Name="Free cam (shift + P)",Callback=function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/Robobo2022/script/main/Freecam.lua"))();
end});
FunTab:AddButton({Name="Server Crasher",Callback=function()
	while task.wait() do
		game:GetService("ReplicatedStorage").Events.Reset:FireServer();
		game:GetService("ReplicatedStorage").Events.Respawn:FireServer();
	end
end});
TeleportTab:AddButton({Name="Main Game",Callback=function()
	local TeleportService = game:GetService("TeleportService");
	GameId = 9872472334;
	TeleportService:Teleport(GameId, game.Players.LocalPlayer);
end});
TeleportTab:AddButton({Name="Casual",Callback=function()
	local TeleportService = game:GetService("TeleportService");
	GameId = 10662542523;
	TeleportService:Teleport(GameId, game.Players.LocalPlayer);
end});
TeleportTab:AddButton({Name="Social Space",Callback=function()
	local TeleportService = game:GetService("TeleportService");
	GameId = 10324347967;
	TeleportService:Teleport(GameId, game.Players.LocalPlayer);
end});
TeleportTab:AddButton({Name="Big Team",Callback=function()
	local TeleportService = game:GetService("TeleportService");
	GameId = 10324346056;
	TeleportService:Teleport(GameId, game.Players.LocalPlayer);
end});
TeleportTab:AddButton({Name="Team DeathMatch",Callback=function()
	local TeleportService = game:GetService("TeleportService");
	GameId = 110539706691;
	TeleportService:Teleport(GameId, game.Players.LocalPlayer);
end});
local MiscTab1 = MiscTab:AddSection({Name="KeyBinds"});
local FunTab1 = FunTab:AddSection({Name="KeyBinds"});
MiscTab:AddBind({Name="Random Vote",Default=Enum.KeyCode.X,Hold=false,Callback=function()
...
FunTab:AddBind({Name="Random Emote",Default=Enum.KeyCode.Z,Hold=false,Callback=function()
	local number = math.random(4);
	local ohString1 = number;
	game:GetService("ReplicatedStorage").Events.Emote:FireServer(ohString1);
end});
MiscTab:AddBind({Name="Respawn",Default=Enum.KeyCode.R,Hold=false,Callback=function()
	game:GetService("ReplicatedStorage").Events.Respawn:FireServer();
end});
ESPTab:AddToggle({Name="Esp",Default=true,Callback=function(Value)
	getgenv().toggleespmpt = Value;
end});
ESPTab:AddColorpicker({Name="Colour",Default=Color3.fromRGB(255, 255, 255),Callback=function(Value)
...
TestTab:AddToggle({Name="Spam Chat",Default=false,Callback=function(Value)
	autochat1 = Value;
	autochat();
end});
...
OrionLib:MakeNotification({Name="Join Discord",Content="Join the Discord Copied in your clip Board",Image="rbxassetid://4483345998",Time=5});
OrionLib:Init();
